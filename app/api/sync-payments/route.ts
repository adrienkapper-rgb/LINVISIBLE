...
export async function POST(request: NextRequest) {
  try {
    ...
    const typedPendingOrders = (pendingOrders || []) as OrderRow[]
    for (const order of typedPendingOrders) {
      results.checked++
      ...
      try {
        ...
        if (paymentIntent.status === 'succeeded') {
          ...
          const existingPayment = existingPaymentData as { id: string; status: string } | null
          ...
          if (!existingPayment || existingPayment.status !== 'succeeded') {
            ...
            if (mode === 'sync') {
              ...
              if (existingPayment) {
                await supabase
                  .from('payments')
                  .update({ ...paymentData, updated_at: new Date().toISOString() } as never)
    ...
    if (!fetchError2 && ordersWithoutPI && ordersWithoutPI.length > 0) {
      ...
      const typedOrdersWithoutPI = ordersWithoutPI as OrderRow[]
      for (const order of typedOrdersWithoutPI) {
        try {
          ...
          if (matchingPI && mode === 'sync') {
            ...
            await supabase
              .from('orders')
              .update({ stripe_payment_intent_id: matchingPI.id } as never)
              .eq('id', order.id)
...
export async function GET() {
  try {
    ...
    const typedStats = (stats || []) as Pick<OrderRow, 'status'>[]
    const statusCounts = typedStats.reduce((acc, order) => {
    ...
    const typedRecentPending = (recentPending || []) as Pick<OrderRow, 'id' | 'order_number' | 'created_at' | 'stripe_payment_intent_id'>[]
    const pendingWithPI = typedRecentPending.filter(o => o.stripe_payment_intent_id)
    const pendingWithoutPI = typedRecentPending.filter(o => !o.stripe_payment_intent_id)

    return NextResponse.json({
      stats: statusCounts,
      recentPending: {
        total: typedRecentPending.length,
